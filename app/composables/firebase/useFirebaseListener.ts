...
export function useFirebaseListener({
  ...
  const handleSnapshot = (snapshot: DocumentSnapshot<DocumentData>) => {
    ...
    if (snapshotData) {
      devLog(`[${storeIdForLogging}] Snapshot received`, {
        ...
        tokensLength: Array.isArray(snapshotData.tokens)
          ? snapshotData.tokens.length
          : "not array",
      ...
      devLog(
        `[${storeIdForLogging}] Snapshot data is null/undefined. Clearing state.`
      );
      resetStore(store);
      if (customOnSnapshot) {
  ...
  const handleError = (error: FirestoreError) => {
    ...
    if (error.code === "permission-denied" && unsubscribe.value) {
      devWarn(
        `[${storeIdForLogging}] Permission denied, unsubscribing and clearing state`
      );
      unsubscribe.value();
  ...
  const updateListeners = (newKeys: string[], oldKeys: string[] = []) => {
    ...
    for (const key of oldKeys) {
      if (!newKeys.includes(key) || !shouldInclude(key)) {
        ...
        const { [key]: _u, ...restUnsubs } = unsubscribes.value;
        unsubscribes.value = restUnsubs;

        const { [key]: _s, ...restStores } = stores.value;
        stores.value = restStores as T;
      }
    }
    // Add listeners for new keys
    for (const key of newKeys) {
      const store = stores.value[key];
      if (!store && shouldInclude(key)) {
        devLog(`Adding listener for key: ${key}`);
        if (store) {
          createListener(key, store);
        }
      }
    }
  };
