...
export function useHideoutData() {
  ...
  const {
    ...
    error,
    loading,
  } = useTarkovHideoutQuery(currentGameMode);
  // Reactive state
  ...
  const buildHideoutGraph = (stations: HideoutStation[]) => {
    ...
    stations.forEach((station: HideoutStation) => {
      station.levels.forEach((level: HideoutLevel) => {
        safeAddNode(newGraph, level.id);
        level.stationLevelRequirements?.forEach(
          (requirement: StationLevelRequirement) => {
            if (requirement?.station?.id) {
              ...
              const requiredStation = stations.find(
                (s: HideoutStation) => s.id === requirement.station.id
              );
              const requiredLevel = requiredStation?.levels.find(
                (l: HideoutLevel) => l.level === requirement.level
              );
              if (requiredLevel?.id) {
   ...
   */
  const extractItemRequirements = (
    modules: HideoutModule[]
  ): NeededItemHideoutModule[] => {
    const neededItems: NeededItemHideoutModule[] = [];
    modules.forEach((module: HideoutModule) => {
      module.itemRequirements?.forEach((req: ItemRequirement) => {
        if (req?.item?.id) {
          neededItems.push({
            id: req.id,
    ...
    (newResult) => {
      try {
        if (newResult?.hideoutStations) {
          ...
          const newModules = createHideoutModules(
            newResult.hideoutStations,
            newGraph
          );
  ...
  const stationsByName = computed(() => {
    ...
    hideoutStations.value.forEach((station: HideoutStation) => {
  ...
  const modulesByStation = computed(() => {
    ...
    hideoutModules.value.forEach((module: HideoutModule) => {
  ...
  const maxStationLevels = computed(() => {
    ...
    hideoutStations.value.forEach((station: HideoutStation) => {
      maxLevels[station.id] = Math.max(
        ...station.levels.map((level: HideoutLevel) => level.level)
      );
    });
    return maxLevels;
  });
  ...
  const getStationById = (stationId: string): HideoutStation | undefined => {
    return hideoutStations.value.find(
      (station: HideoutStation) => station.id === stationId
    );
  };
  /**
  ...
  const getModuleById = (moduleId: string): HideoutModule | undefined => {
    return hideoutModules.value.find(
      (module: HideoutModule) => module.id === moduleId
    );
  };
  /**
  ...
  const getItemsForModule = (moduleId: string): NeededItemHideoutModule[] => {
    return neededItemHideoutModules.value.filter(
      (item: NeededItemHideoutModule) => item.hideoutModule.id === moduleId
    );
  };
  /**
   * Get modules that require a specific item
   */
  const getModulesRequiringItem = (
    itemId: string
  ): NeededItemHideoutModule[] => {
    return neededItemHideoutModules.value.filter(
      (item: NeededItemHideoutModule) => item.item.id === itemId
    );
  };
  /**
  ...
  const getTotalConstructionTime = (moduleId: string): number => {
    ...
    module.predecessors.forEach((prerequisiteId: string) => {
