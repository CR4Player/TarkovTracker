import { firestore, fireuser } from "@/plugins/firebase.client";
import {
  doc,
  onSnapshot,
  setDoc,
...
export type StoreWithFireswapExt<StoreType extends Store> = StoreType &
    ...
    fireswapSettings.forEach((fireswapSetting, fsIndex) => {
      if (fireswapSetting.document && fireswapSetting.localKey) {
        ...
        const uploadFunction = (currentStateSnapshot: StateTree) => {
          ...
          if (
            !stateToSave ||
            !Object.keys(stateToSave as Record<string, unknown>).length
          ) {
            ...
            if (store.saving && typeof store.saving === "object") {
              Object.keys(store.saving).forEach(
                (k) => (store.saving[k] = false)
              );
            }
            return;
          }
          ...
          if (fireuser.loggedIn && fireuser.uid) {
            try {
              const docRef = parseDoc(fireswapSetting.document);
              setDoc(docRef, stateToSave as Record<string, unknown>, {
                merge: true,
              })
                .then(() => {
                  /* Intentionally ignored */
...
// No-op Nuxt plugin to satisfy plugin loader while keeping named exports
